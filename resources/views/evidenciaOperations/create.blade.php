                ...
                @elseif(\Illuminate\Support\Facades\Auth::user()->rol == 'Estudiante' & \Illuminate\Support\Facades\Auth::user()->disponibilidad == 'No'  & auth()->user()->bitacoras->first()->estado != 'Finalizada' )
                    ...
                    @foreach(auth()->user()->bitacoras as $bita)
                        ...
                        @if(($bita->estado)=='Activa')
                            ...
                            @if(\Illuminate\Support\Facades\DB::select('SELECT bitacora_id FROM avances WHERE bitacora_id = ?',[$bita->id]))
                                ...
                                <form class="bg-white py-3 px-4 shadow rounded" method="POST"
                                    ...
                                    <div class="form-group">
                                        <label class="custom-select-lg" for="name_evid"> Nombre Evidencia </label>
                                    ...
                                    @foreach(auth()->user()->bitacoras as $bita)
                                        <div class="form-group">
                                            <label class="custom-select-lg" for="avance_id"> Avances en la
                                                Bitacora </label>

                                            <select class="form-control shadow-sm custom-select" name="avance_id">
                                                @foreach($bita->avances as $avance)
                                                    <option class="custom-select-lg" value="{{$avance->id}}">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <br>
                                        <input accept="image/jpg, image/png, application/pdf, .docx" class=""
                                               name="archivo"
                                               type="file" required>
                                    </div>
                                    <br>
                            ...
                            @else
                                <div class="display-3 text-center">
                                    <p>No tiene Avances al cual añadir una Evidencia.</p>
                                </div>
